...
module.exports = {
  ...
  async getByUser(request, response) {
    try {
      ...
      return response.status(500).json({
        notification:
          " Internal server error while trying to get Perfil by user",
      });
    }
  },
  ...
  async update(request, response) {
    try {
      ...
      return response
        .status(200)
        .json({ notification: "Perfil Updated Sucessfully" });
    } catch (error) {
  ...
  async delete(request, response) {
    try {
      ...
      if (result == 0) {
        return response
          .status(400)
          .json({ notification: "Perfil id not found" });
      }
